...
public class ToolListener implements Listener {
    ...
    public void onBlockPlace(BlockPlaceEvent e) {
        ...
        if (SlimefunManager.isItemSimiliar(item, SlimefunItems.BASIC_CIRCUIT_BOARD, true)) {
            ...
            if (e.getBlock().getY() != e.getBlockAgainst().getY()) {
                ...
                e.getBlock().getY() != e.getBlockAgainst().getY()) {
            Messages.local.sendTranslation(e.getPlayer(), "machines.CARGO_NODES.must-be-placed", true);
            e.setCancelled(true);
        }
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (StringUtils.equals(block2.getType().toString(), "SAPLING", "WOOD_PLATE", "STONE_PLATE", "IRON_PLATE", "GOLD_PLATE")) {
            SlimefunItem slimefunItem = BlockStorage.check(e.getBlock().getRelative(BlockFace.UP));
            if (slimefunItem != null && !(slimefunItem instanceof me.mrCookieSlime.Slimefun.Objects.SlimefunItem.HandledBlock)) {
                if (SlimefunItem.blockhandler.containsKey(slimefunItem.getID())) {
                    allow = SlimefunItem.blockhandler.get(slimefunItem.getID()).onBreak(e.getPlayer(), block2, slimefunItem, UnregisterReason.PLAYER_BREAK);
                }
                if (allow) {
        ...
        if (sfItem != null && !(sfItem instanceof me.mrCookieSlime.Slimefun.Objects.SlimefunItem.HandledBlock)) {
            if (SlimefunItem.blockhandler.containsKey(sfItem.getID())) {
