...
public class Elevator {
    ...
    public static void openEditor(Player p, final Block b) {
        ...
        menu.addMenuClickHandler(4, (p12, arg1, arg2, arg3) -> {
            p12.closeInventory();
            p12.sendMessage("");
            p12.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4&l>> &e请在聊天栏内输入你想要设置的楼层名称!"));
            p12.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4&l>> &e(支持颜色代码!"));
            p12.sendMessage("");
            ...
            MenuHelper.awaitChatInput(p12, (p1, message) -> {
                ...
                p1.sendMessage("");
                p1.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4&l>> &e成功设置楼层名称:"));
    ...
    public static void openDialogue(Player p, Block b) {
        ...
        for (int y = b.getWorld().getMaxHeight(); y > 0; y--) {
            ...
            if (BlockStorage.check(block, "ELEVATOR_PLATE")) {
                ...
                if (block.getY() == b.getY()) {
                    ...
                    tellraw.addHoverEvent(TellRawMessage.HoverAction.SHOW_TEXT, "\n&e这是你目前所在的楼层:\n&r" + floor + "\n");
                } else {
        ...
        if (index > 2) {
            (new CustomBookOverlay("电梯", "Slimefun", tellraw)).open(p);
        } else {
